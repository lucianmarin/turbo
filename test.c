...
TurboObject* t_Ctx = NULL;
TurboObject* t_impl_Ctx___init__(int argc, TurboObject** args);
TurboObject* t_impl_Ctx___enter__(int argc, TurboObject** args);
TurboObject* t_impl_Ctx___exit__(int argc, TurboObject** args);
TurboObject* t_impl_test_with(int argc, TurboObject** args);
TurboObject* t_test_with = NULL;
...
TurboObject* t_impl_Ctx___init__(int argc, TurboObject** args) {
    TurboObject* t_self = (argc > 0) ? args[0] : turbo_none;
    turbo_setattr(t_self, "entered", turbo_false);
    turbo_setattr(t_self, "exited", turbo_false);
    turbo_setattr(t_self, "info", make_str(""));
    return turbo_none;
}

TurboObject* t_impl_Ctx___enter__(int argc, TurboObject** args) {
    TurboObject* t_self = (argc > 0) ? args[0] : turbo_none;
    turbo_setattr(t_self, "entered", turbo_true);
    return make_int("42");
    return turbo_none;
}
...
TurboObject* t_impl_Ctx___exit__(int argc, TurboObject** args) {
    TurboObject* t_self = (argc > 0) ? args[0] : turbo_none;
    TurboObject* t_a = (argc > 1) ? args[1] : turbo_none;
    TurboObject* t_b = (argc > 2) ? args[2] : turbo_none;
    TurboObject* t_c = (argc > 3) ? args[3] : turbo_none;
    turbo_setattr(t_self, "exited", turbo_true);
    if (turbo_is_truthy(turbo_eq(t_a, turbo_none))) {
        turbo_setattr(t_self, "info", make_str("ok"));
    } else {
        turbo_setattr(t_self, "info", make_str("exc"));
    }
    return turbo_false;
    return turbo_none;
}
...
TurboObject* t_impl_test_with(int argc, TurboObject** args) {
    ...
    turbo_call(t_print, 1, (TurboObject*[]){make_str("With statement:")});
    t_c = turbo_call(t_Ctx, 0, NULL);
    {
        TurboObject* _cm = t_c;
        t_val = turbo_call_method(_cm, "__enter__", 0, NULL);
        int _exc_had = 0;
        TurboObject* _exc_val = turbo_none;
        jmp_buf* _saved_jmp = turbo_exception_jmp;
        jmp_buf _try_jmp;
        turbo_exception_jmp = &_try_jmp;
        turbo_exception_value = turbo_none;
        if (setjmp(_try_jmp) == 0) {
            turbo_call(t_print, 2, (TurboObject*[]){make_str("  val:"), t_val});
            turbo_call(t_print, 2, (TurboObject*[]){make_str("  entered:"), turbo_getattr(t_c, "entered")});
        } else {
            _exc_had = 1;
            _exc_val = turbo_exception_value;
        }
        turbo_exception_jmp = _saved_jmp;
        if (_exc_had) {
            TurboObject* _exc_type = _exc_val;
            if (_exc_val->type == TYPE_INSTANCE) _exc_type = _exc_val->inst_val.class_obj;
            if (!turbo_is_truthy(turbo_call_method(_cm, "__exit__", 3, (TurboObject*[]){_exc_type, _exc_val, turbo_none}))) {
                turbo_raise(_exc_val);
            }
        } else {
            turbo_call_method(_cm, "__exit__", 3, (TurboObject*[]){turbo_none, turbo_none, turbo_none});
        }
    }
    turbo_call(t_print, 2, (TurboObject*[]){make_str("  exited:"), turbo_getattr(t_c, "exited")});
    turbo_call(t_print, 2, (TurboObject*[]){make_str("  info:"), turbo_getattr(t_c, "info")});
    return turbo_none;
}
...
void turbo_main(void) {
    ...
    t_Ctx = make_class("Ctx");
    turbo_class_add_method(t_Ctx, "__init__", t_impl_Ctx___init__);
    turbo_class_add_method(t_Ctx, "__enter__", t_impl_Ctx___enter__);
    turbo_class_add_method(t_Ctx, "__exit__", t_impl_Ctx___exit__);
    t_test_with = make_func(t_impl_test_with, "test_with");
